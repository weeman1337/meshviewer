define(["sorttable", "snabbdom", "d3-interpolate", "helper", "utils/node"], function (
  SortTable,
  V,
  d3Interpolate,
  helper,
  ...
  function showStatImg(o, d) {
    var subst = {
      "{NODE_ID}": d.node_id,
      "{NODE_NAME}": d.hostname.replace(/[^a-z0-9\-]/gi, "_"),
  ...
  return function (el, d, linkScale, nodeDict) {
    ...
    function renderNeighbourRow(n) {
      var icons = [
        V.h("span", {
          props: {
            className: "icon ion-" + (n.link.type.indexOf("wifi") === 0 ? "wifi" : "share-alt"),
            title: _.t(n.link.type),
          },
        }),
      ];
    ...
    self.render = function render() {
      ...
      children.push(V.h("tr", [V.h("th", _.t("node.gateway")), showGateway(d)]));
